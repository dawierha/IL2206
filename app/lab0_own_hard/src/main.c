...
{
    int btn_reg;
    btn_reg = IORD_ALTERA_AVALON_PIO_DATA(DE2_PIO_KEYS_BASE);
    btn_reg = (~btn_reg) & 0xf;
    switch (btn_reg)
    {
        case 1:
...
{ 
    ...
    IOWR_ALTERA_AVALON_PIO_EDGE_CAP(DE2_PIO_KEYS_BASE, 0);
    /* reset interrupt capability for the Button PIO. */
    IOWR_ALTERA_AVALON_PIO_IRQ_MASK(DE2_PIO_KEYS_BASE, 0xf);
}
...
int main ()
{
    ...
    IOWR_ALTERA_AVALON_PIO_EDGE_CAP(DE2_PIO_KEYS_BASE, 0x0);
    // Register the ISR for buttons
    alt_irq_register(DE2_PIO_KEYS_IRQ, NULL, Key_InterruptHandler);
